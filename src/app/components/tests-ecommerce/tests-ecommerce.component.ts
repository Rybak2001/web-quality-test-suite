...
@Component({
  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        <div>
          <div class="breadcrumb"><a routerLink="/">Dashboard</a> / Tests E-Commerce</div>
          <h1>Tests — SurtiBolivia Tienda</h1>
          <p class="subtitle">Suite completa de pruebas para la plataforma e-commerce</p>
        </div>
      ...
      <div class="summary-grid">
        <div class="summary-card">
          <mat-icon class="summary-icon">folder_open</mat-icon>
  ...
  styles: [`
    ...
    .summary-icon { color: #5b4fc7; font-size: 28px; width: 28px; height: 28px; }
    ...
    .section-header { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 2px solid #ede9fe; }
    .section-header mat-icon { color: #5b4fc7; }
...
export class TestsEcommerceComponent implements OnInit, OnDestroy {
  suites: TestSuite[] = [];
  running = false;
  ...
  ngOnInit(): void {
    // Ecommerce-related suite IDs: 1 (Tienda), 3 (Rendimiento), 4 (A11y), 5 (Seguridad), 6 (SEO), 7 (Unit E-Commerce)
    const ecommerceIds = ['1', '3', '4', '5', '6', '7'];
    this.sub = this.testService.getSuites().subscribe(all => {
      this.suites = all.filter(s => ecommerceIds.includes(s.id));
      this.buildSections();
      this.updateCounts();
    });
  ...
  buildSections(): void {
    const map: Record<string, { label: string; icon: string; suites: TestSuite[]; tests: TestCase[] }> = {
      ...
      accessibility: { label: 'Tests de Accesibilidad', icon: 'accessibility_new', suites: [], tests: [] },
    };
